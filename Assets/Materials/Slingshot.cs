...
public class Slingshot : MonoBehaviour {
	...
	public float velocityMultiplyer = 4.0f;
	...
	void OnMouseDown() {
		...
		// Set position to the launch point
		projectile.transform.position = launchPos;

		// make our projectile kinematic (for now)
		projectile.GetComponent<Rigidbody>().isKinematic = true;

		// set the camera poi to this new created projectile
	...
	void Update() {
		...
		if(Input.GetMouseButtonUp(0)) {
			...
			projectile.GetComponent<Rigidbody>().velocity = -mouseDelta * velocityMultiplyer;
